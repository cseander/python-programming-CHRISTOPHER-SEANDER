...
class Rectangle(GeometryShape):
    ...
    def side_horizontal(self, value: float) -> None:
        if not isinstance(value, (float, int)):
            raise TypeError(f"side_horizontal has to be a float or int, not {type(value)}")
    ...
    def side_vertical(self, value: float) -> None:
        if not isinstance(value, (float, int)):
            raise TypeError(f"side_vertical has to be a float or int, not {type(value)}")
    ...
    def circumference(self) -> float:
        ...
        return 2 * (self.side_horizontal + self.side_vertical)
    ...
    def is_inside(self, x: float, y: float) -> bool:
        ...
        xy_inside = ((self.x - (self.side_horizontal / 2)) <= x <= (self.x + (self.side_horizontal / 2)),
                     (self.y - (self.side_vertical / 2)) <= y <= (self.y + (self.side_vertical / 2)))
        return all(xy_inside)
    
    def __repr__(self) -> str:
        return f"rectangle with area: {round(self.area(), 2)}, circumference: {round(self.circumference(), 2)} and center point in {self.x, self.y}"